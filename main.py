...
class Canvas3D:
    ...
    def draw_with_zbuffer(self, canvas, polygons):
        # Инициализация z-буфера и кадрового буфера
        ...
        for poly in polygons:
            ...
            # Разбиваем на треугольники (если >3 вершин)
            triangles = []
            if n == 3:
                triangles = [projected]
            elif n >= 4:
        ...
        for y in range(self.height):
            for x in range(self.width):
                if colorbuffer[y][x] != '#ffffff':
                    canvas.create_rectangle(x, y, x+1, y+1, fill=colorbuffer[y][x], outline='')
...
class GraphicsApp:
    def __init__(self, root):
        ...
        scale_slider.pack(padx=5, pady=5)
        ...
        # Создание объектов согласно варианту 9
        self.polygons = self.create_objects()
        self.canvas3d = Canvas3D(800, 600)
        self.redraw()

    def create_objects(self):
        # а) Куб — 8 вершин, 6 граней
        cube_v = [
            (-4, -1, -1), (-2, -1, -1), (-2, 1, -1), (-4, 1, -1),  # задняя грань
            (-4, -1, 1), (-2, -1, 1), (-2, 1, 1), (-4, 1, 1)      # передняя грань
        ]
        cube_faces = [
            [0,1,2,3], [4,5,6,7], [0,1,5,4],
            [2,3,7,6], [0,3,7,4], [1,2,6,5]
        ]
        cube = [{"vertices_3d": [cube_v[i] for i in face], "color": "#ADD8E6"} for face in cube_faces]

        # б) Пирамида — 5 вершин, 5 граней
        ...
        pyramid = [{"vertices_3d": face, "color": "#90EE90"} for face in pyramid_faces]

        # з) Произвольный объект — невыпуклый 4-угольник
        arbitrary = [{
